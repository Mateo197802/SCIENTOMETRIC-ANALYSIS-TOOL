...
# ─── PATHS ────────────────────────────────────────────────────────────────────
BASE_DIR = r"C:\Users\intel\OneDrive - yachaytech.edu.ec\Escritorio\MIT\Scientometric Analysis Tool\SCIENTOMETRIC ANALYSIS TOOL"
SENT_DIR      = os.path.join(BASE_DIR, "SENT")
MAQUINA_DIR   = os.path.join(SENT_DIR, "output_maquina")
WEB_DIR       = os.path.join(SENT_DIR, "output_web")
GRAFICAS_DIR  = os.path.join(SENT_DIR, "graficas")
MASTER_CSV    = os.path.join(BASE_DIR, "A1", "PYTHON", "TOOL", "data", "MASTER_AUTHOR_TABLE.csv")
...
def main():
    ...
    stats_df.to_csv(os.path.join(SENT_DIR, "evaluation_summary.csv"), index=False)
    field_df.to_csv(os.path.join(SENT_DIR, "field_completeness_detail.csv"), index=False)
    ...
    print(f"{'='*70}")
    print(f"\nAll outputs saved to: {SENT_DIR}")
    print(f"  |-- output_maquina/   ({len(os.listdir(MAQUINA_DIR))} files)")
    print(f"  |-- output_web/       ({len(os.listdir(WEB_DIR))} files)")
    print(f"  +-- graficas/         (5 graphs)")
